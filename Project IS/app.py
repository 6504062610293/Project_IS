...
# กำหนดโฟลเดอร์สำหรับเก็บไฟล์ที่ User อัปโหลด
UPLOAD_FOLDER = 'uploads'
ALLOWED_EXTENSIONS = {'csv'}
app.config['UPLOAD_FOLDER'] = UPLOAD_FOLDER

# ตรวจสอบว่ามีโฟลเดอร์ uploads หรือยัง ถ้ายังไม่มีก็สร้างใหม่
os.makedirs(UPLOAD_FOLDER, exist_ok=True)

def allowed_file(filename):
    """
    ฟังก์ชันตรวจสอบนามสกุลไฟล์ที่อนุญาต
    คืนค่า True ถ้าใช่นามสกุลที่อนุญาต (.csv)
    """
...
def index():
    """
    Route เริ่มต้น (Homepage) แสดงหน้าอัพโหลด
    """
...
def upload_file():
    """
    Route สำหรับรับไฟล์อัปโหลดจากฟอร์ม
    """
    # ตรวจสอบว่า request มีไฟล์แนบมาหรือไม่
    ...
    # ถ้าผู้ใช้ไม่ได้เลือกไฟล์
    if file.filename == '':
        flash('ยังไม่ได้เลือกไฟล์')
        return redirect(url_for('index'))
    
    # ถ้าไฟล์มีนามสกุลถูกต้อง
    if file and allowed_file(file.filename):
        # ทำให้ชื่อไฟล์ปลอดภัยจากการถูกโจมตีผ่าน Path
        ...
        # เมื่อบันทึกเสร็จ ให้ redirect ไปยังหน้าประมวลผล (Preprocessing) โดยส่งชื่อไฟล์ไปด้วย
        return redirect(url_for('preprocess', filename=filename))
    else:
        flash('อนุญาตเฉพาะไฟล์ .csv เท่านั้น')
        return redirect(url_for('index'))
...
def preprocess(filename):
    ...
    if not os.path.exists(filepath):
        flash('ไม่พบไฟล์ที่ระบุ กรุณาอัปโหลดใหม่')
        return redirect(url_for('index'))
        
    try:
        # เรียกใช้ฟังก์ชันจาก module ของเรา
        # รับค่า steps_info ที่เป็น dictionary เพื่อไปแสดงผล HTML
        # รับค่า df_scaled ซึ่งคือข้อมูลที่พร้อม Train โมเดล 
        # (df_scaled เราจะบันทึกเป็นไฟล์ใหม่เพื่อจำลองว่าพร้อมส่งต่อ)
        ...
        return render_template('preprocessing.html', steps=steps_info, filename=filename)
...
def train_models():
    ...
    try:
        ...
        # สมมติฐานว่าคอลัมน์ชื่อ diagnosis เป็น Target เสมอ ตามกระบวนการ Preprocessing 
        # (ซึ่งได้ถูกแปลงเป็น 0, 1 แล้ว)
        if 'diagnosis' not in df.columns:
            flash("ไม่พบคอลัมน์วินิจฉัยโรค ('diagnosis') ในชุดข้อมูลนี้ ไม่สามารถจำแนกประเภทได้")
            return redirect(url_for('index'))
        ...
        # หาก Train สำเร็จ เก็บข้อมูล Result (เช่นความแม่นยำ) ไปแสดงผลหน้าใหม่
        # โดยการส่ง object เข้า Template ตรงๆ ต้องสร้างหน้าใหม่
